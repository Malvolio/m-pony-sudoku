(function () {
  ...
  $("#game .grid tr td").on("click", function (event) {
    ...
    grid[i].push(peerId);
    $(
      "#game .grid tr:eq(" + (6 - grid[i].length) + ") td:eq(" + i + ") .slot"
    ).addClass("filled");
  ...
  function start() {
    ...
    peer.on("open", function () {
      $("#game .alert p")
        ...
        .append(
          $('<span class="pull-right"></span>').text("Peer ID: " + peerId)
        );
  ...
  $("#game .grid td").on("mouseenter", function () {
    $("#game .grid tr td:nth-child(" + ($(this).index() + 1) + ")").addClass(
      "hover"
    );
  });
  $("#game .grid td").on("mouseleave", function () {
    $("#game .grid tr td:nth-child(" + ($(this).index() + 1) + ")").removeClass(
      "hover"
    );
  });
})();
